class ForecastWeatherResponse {
  ...
  ForecastWeatherResponse.fromJson(dynamic json) {
    ...
    if (json['list'] != null) {
      list = [];
      json['list'].forEach((v) {
        list?.add(ForcastList.fromJson(v));
      });
    }
  ...
  City? city;
ForecastWeatherResponse copyWith({  String? cod,
  num? message,
  num? cnt,
  List<ForcastList>? list,
  City? city,
}) => ForecastWeatherResponse(  cod: cod ?? this.cod,
...
class ForcastList {
  ...
  ForcastList.fromJson(dynamic json) {
    dt = json['dt'];
